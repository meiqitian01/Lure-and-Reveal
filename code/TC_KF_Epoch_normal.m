%在输出中多加了一个detection_result
function [K_matrix_denominator, K_matrix, lamda,delta_KF_x, est_C_b_e_new,est_v_eb_e_new,est_r_eb_e_new,est_IMU_bias_new,...
            est_clock_new,P_matrix_new] = TC_KF_Epoch_normal(GNSS_measurements,...
...
for j = 1:no_meas
    ...
    delta_r = GNSS_measurements(j,3:5)' - est_r_eb_e_old; %第j行，第三到五列：Satellite ECEF position (m)- prior estimated ECEF user position (m)
...
K_matrix_denominator = H_matrix * P_matrix_propagated * H_matrix' + R_matrix;
K_matrix = P_matrix_propagated * H_matrix' * inv(K_matrix_denominator);
...
delta_KF_x = K_matrix * delta_z;
x_est_new = x_est_propagated + delta_KF_x;
