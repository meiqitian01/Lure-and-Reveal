...
%                    along body-frame axes, averaged over time interval (m/s^2)时间间隔之内取平均值
...
%
% Outputs:
%   meas_f_ib_b      output specific  of body frame w.r.t. ECEF frame, resolved
...
% 如果量化级别大于0，将未量化输出加上剩余误差进行量化，计算新的剩余。
if IMU_errors.accel_quant_level>0
    % disp("IMU_errors.accel_quant_level>0");
    % disp("uq_f_ib_b:");
    % disp(uq_f_ib_b);
    % disp("old_quant_residuals(1:3):");
    % disp(old_quant_residuals(1:3));
    % disp("IMU_errors.accel_quant_level):");
    % disp(IMU_errors.accel_quant_level);

    %量化加速度计输出：未量化的加速度输出加上前一次量化中的残差，除以量化级别，round四舍五入
    meas_f_ib_b = IMU_errors.accel_quant_level * round((uq_f_ib_b +...
        old_quant_residuals(1:3)) / IMU_errors.accel_quant_level);
    %重新计算量化残差，等于未量化输出加上剩余误差减去上一步的量化加速度计输出
